...
# Колонки которые ДОЛЖНЫ быть в исходном файле (профит вычисляется автоматически,
# поэтому его наличие в файле не требуем)
REQUIRED_IN_FILE = [c for c in REQUIRED_COLUMNS if c != "профит"]
OPTIONAL_COLUMNS = ["линия", "формат", "период", "команда_ставки", "заметки"]
...
# ============================================================
@st.cache_data
def load_data(file_bytes: bytes) -> tuple[pd.DataFrame, int, int]:
    """
    Читает Excel, оставляет только строки с заполненной датой.
    Возвращает (df, total_rows_in_file, dropped_rows).

    Лист ищем гибко: сначала "Ставки", потом любой вариант написания,
    иначе берём первый лист в файле. Так дашборд переварит и файлы
    сделанные не нашим ботом (главное чтобы колонки совпадали).
    """
    xls = pd.ExcelFile(io.BytesIO(file_bytes))
    sheet_names = xls.sheet_names

    # Ищем лист "Ставки" в любом регистре / с пробелами
    target_sheet = None
    for name in sheet_names:
        if str(name).strip().lower() == "ставки":
            target_sheet = name
            break
    ...
    # Чистим имена колонок от лишних пробелов (частая проблема при ручном копировании)
    df.columns = [str(c).strip() for c in df.columns]

    total_rows = len(df)

    # Если нет даже колонки "дата" — вернём как есть, валидатор ниже выдаст
    # понятную ошибку про недостающие колонки
    ...
    # Если после конвертации остались NaT (нераспознанные даты) — тоже убираем
    df = df.dropna(subset=["дата"]).reset_index(drop=True)
    dropped = total_rows - len(df)
    ...
    return df, total_rows, dropped


def validate_columns(df: pd.DataFrame):
    missing = [c for c in REQUIRED_IN_FILE if c not in df.columns]
...
def aggregate(df: pd.DataFrame, by) -> pd.DataFrame:
    ...
    agg = df.groupby(by_list, dropna=False).agg(
        ставок=("профит", "count"),
        оборот=("ставка", "sum"),
        профит=("профит", "sum"),
    )
    if len(agg) == 0:
        return pd.DataFrame(columns=by_list + ["ставок", "оборот", "профит", "ROI%"])
    # Защита от деления на 0 — если оборот = 0 (например, у сигналов где ставка 0),
    # ROI не определён, ставим 0
    agg["ROI%"] = agg.apply(
        lambda r: round(r["профит"] / r["оборот"] * 100, 1) if r["оборот"] > 0 else 0.0,
        axis=1,
    )
...
def aggregate_winrate(df: pd.DataFrame, by) -> pd.DataFrame:
    ...
    agg = df.groupby(by_list, dropna=False).apply(calc, include_groups=False).reset_index()
    # Защита от пустого результата (все строки с NaN в группировке или нет данных вообще)
    if len(agg) == 0 or "Винрейт%" not in agg.columns:
        return pd.DataFrame(columns=by_list + ["ставок", "WIN", "LOSE", "RETURN", "Винрейт%"])
...
df_all, total_rows, dropped_rows = load_data(uploaded.getvalue())
ok, missing = validate_columns(df_all)
if not ok:
    st.error(f"❌ В файле не хватает обязательных колонок: {', '.join(missing)}")
    st.info(
        "**Колонки, которые есть в загруженном файле:**\n\n"
        + ", ".join(f"`{c}`" for c in df_all.columns)
    )
    st.caption(
        "💡 Названия колонок должны точно совпадать (на русском, в нижнем регистре). "
        "Проверь что нет лишних пробелов или другого написания. "
        "Самый надёжный способ получить правильный файл — экспорт через бота (/export)."
    )
    st.stop()

# Диагностика загрузки — если что-то отвалилось при загрузке, сразу видно
if dropped_rows > 0:
    st.warning(
        f"⚠️ Из {total_rows} строк в файле {dropped_rows} отброшено "
        f"(пустая дата). Осталось {len(df_all)} строк для анализа."
    )

if len(df_all) == 0:
    st.error(
        f"В файле {total_rows} строк, но **ни одна** не подходит для анализа "
        "(у всех пустая или невалидная дата). Проверь файл."
    )
    st.stop()
...
# Анализируем только реальные ставки. MODE оставлен как константа "BET"
# чтобы не переписывать все места ниже разом — они просто всегда идут по ветке ставок.
MODE = "BET"
...
# Календарь: даём чуть больше места чем строго от min до max в данных,
# чтобы пользователь видел "сегодня" даже если данные старые.
# Это полезно если только что добавил ставку через бот, но xlsx не переэкспортировал.
from datetime import timedelta, date as date_cls
_today = date_cls.today()
date_data_min = df_all["дата"].min().date()
date_data_max = df_all["дата"].max().date()
# Минимум — данные минус неделя; максимум — максимум данных или сегодня (что больше)
cal_min = date_data_min - timedelta(days=7)
cal_max = max(date_data_max, _today) + timedelta(days=1)
date_range = st.sidebar.date_input(
    "📅 Даты",
    value=(date_data_min, date_data_max),
    min_value=cal_min,
    max_value=cal_max,
)

# Подсказка если в файле данных нет за сегодняшний день
if date_data_max < _today:
    st.sidebar.caption(
        f"💡 В файле данные до {date_data_max.strftime('%d.%m')}. "
        f"Если добавил ставки позже — переэкспортируй xlsx через /export в боте."
    )
...
if len(df) < MIN_N_FOR_RELIABLE:
    st.warning(
        ...
        "Сейчас многое может быть статистическим шумом."
    )
...
# Заголовок и подпись табов (исторически зависели от режима, теперь всегда ставки)
def _t(roi_text: str, winrate_text: str) -> str:
    """Хелпер: вернуть ROI-вариант текста (сигналы убраны, всегда реальные ставки)."""
    return roi_text
