...
with tab_types:
    ...
    render_bar(df, "направление",
               title=_t("ROI по направлению", "Винрейт по направлению"),
...
with tab_heat:
    if MODE == "SIGNAL":
        ...
        heat = df.groupby(["канал", "тип_ставки"]).apply(
            lambda g: pd.Series({
                "ставок": len(g),
        ...
        pivot_metric = heat.pivot(index="канал", columns="тип_ставки", values="Винрейт%")
...
with tab_combo:
    if MODE == "SIGNAL":
        ...
        combo = df.groupby(["канал", "тип_ставки"]).apply(
            lambda g: pd.Series({
                "прогнозов": len(g),
        ...
        if len(combo_f) == 0:
            ...
            with ca:
                ...
                st.dataframe(
                    top[["канал", "тип_ставки", "прогнозов", "WIN", "LOSE", "Винрейт%"]],
            ...
            with cb:
                ...
                st.dataframe(
                    bot[["канал", "тип_ставки", "прогнозов", "WIN", "LOSE", "Винрейт%"]],
                    use_container_width=True, hide_index=True,
                )
    else:
