...
OPTIONAL_COLUMNS = ["линия", "кэф_прогноза", "комиссия", "формат", "тип_записи", "период", "заметки"]
...
if "кэф" in df_all.columns and df_all["кэф"].notna().any():
    kef_min = float(df_all["кэф"].min())
    kef_max = float(df_all["кэф"].max())
...
# Применяем фильтр по кэфу
if kef_range:
    df = df[(df["кэф"] >= kef_range[0]) & (df["кэф"] <= kef_range[1])]
